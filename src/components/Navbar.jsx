...
const Navbar = () => {
  ...
  return (
    <>
      <nav id='nav' className={`fixed z-[1000] w-full left-0 top-0 flex justify-between items-baseline px-[20px] md:px-[50px] lg:px-[120px] py-7 bg-white ${!scroll ? 'lg:bg-transparent lg:shadow-0' : 'lg:shadow-2xl'} transition-all duration-500`}>
      ...
      <div  id='nav' className={`lg:hidden fixed z-[999] w-full ${menu ? 'top-[70px] shadow-2xl' : '-top-[150px] shadow-0'} text-black flex flex-col text-center gap-3 uppercase text-[14px] bg-white py-6 transition-all duration-500`}>
        <NavLink to="/" className='hover:text-sky-500'>Home</NavLink>
        <NavLink to="/about" className='hover:text-sky-500'>About Us</NavLink>
        <NavLink to="/services" className='hover:text-sky-500'>Services</NavLink>
        <NavLink to="/contact" className='hover:text-sky-500'>Contact</NavLink>
